...
class Population:
    ...
    """
    def __init__(self, size, chrom_size, cp, mp, gen_max, fyx_pos_init, variable_ranges=None):
        # 种群信息
        ...
        self.age = 0                       # 种群当前代数
        self.fyx_pos_init= fyx_pos_init  # 无人机初始位置
    ...
    def fitness_func(self, individual_chromosomes):
        ...
        duration, *_ = calculate_smoke_valid_duration(
            ...
            t_drop=t_drop,
            t_burst=t_burst,
            fyx_pos_init=self.fyx_pos_init
        )
        return duration
    ...
    def run(self):
        ...
        best_params = (best_speed, best_theta, best_tdrop, best_tburst)
        return best_params
...
pop = Population(
    ...
    fyx_pos_init=fyx_points[1],
    variable_ranges=ranges
)

# # 近似解
# approx_solution = (80, 285, 12, 5)
# chroms = [
#     encode_param(ranges['drone_speed'], approx_solution[0], CHROMOSOME_BITS),
#     encode_param(ranges['theta'], approx_solution[1], CHROMOSOME_BITS),
#     encode_param(ranges['t_drop'], approx_solution[2], CHROMOSOME_BITS),
#     encode_param(ranges['t_burst'], approx_solution[3], CHROMOSOME_BITS),
# ]
# pop.individuals[0] = chroms
best_params_2 = pop.run()
t_duration_2, t_valid_interval_2, smoke_inf_2 = calculate_smoke_valid_duration(*best_params_2, fyx_points[1])
...
pop = Population(
    ...
    mp=MUTATION_PROB, 
    gen_max=MAX_GENERATIONS,
    fyx_pos_init=fyx_points[2],
    variable_ranges=ranges
)

# # 近似解
# approx_solution = (95.5, 135, 36.8, 9)
# chroms = [
#     encode_param(ranges['drone_speed'], approx_solution[0], CHROMOSOME_BITS),
#     encode_param(ranges['theta'], approx_solution[1], CHROMOSOME_BITS),
#     encode_param(ranges['t_drop'], approx_solution[2], CHROMOSOME_BITS),
...
best_params_3 = pop.run()
t_duration_3, t_valid_interval_3, smoke_inf_3 = calculate_smoke_valid_duration(*best_params_3, fyx_points[2])
...
pop = Population(
    ...
    mp=MUTATION_PROB, 
    gen_max=MAX_GENERATIONS,
    fyx_pos_init=fyx_points[0],
    variable_ranges=ranges
)
best_params_1 = pop.run()
t_duration_1, t_valid_interval_1, smoke_inf_1 = calculate_smoke_valid_duration(*best_params_1, fyx_points[0])
