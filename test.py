...
def calculate_smoke_valid_duration(drone_speed, theta_deg, t_drop, t_burst, fyx_pos_init, step=0.01, t_bias=0):
    ...
    # --- M1运动部分 ---
    M1_initial_point = (19000, 600, 2100)
    M1_speed = 300
...
def demo_func(drone_speed, theta, t_drop, t_burst, fyx_pos_init=(11000, 2000, 1800)):
    t_span, *_ = calculate_smoke_valid_duration(drone_speed, theta, t_drop, t_burst, fyx_pos_init)
    return -(t_span[1] - t_span[0])

pso = PSO(func=demo_func, dim=4, pop=80, max_iter=50, lb=[70, 180, 5, 0], ub=[140, 360, 15, 15], w=0.8, c1=0.5, c2=0.5)
pso.run()
...
plt.ylabel('Best Y')
plt.title('q2:PSO Optimization History')
plt.show()