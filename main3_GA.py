...
def schaffer(p):
    ...
    '''
    total_duration, *_ = calculate_smoke_valid_duration_2(*p)
...
ga = GA(func=schaffer, n_dim=12, size_pop=50, max_iter=400, prob_mut=0.001, lb=[70, 90, 0, 0, 70, 0, 1, 0, 70, 0, 1, 0], ub=[140, 180, 5, 5, 140, 360, 5, 5, 140, 360, 5, 5], precision=1e-7)
best_x, best_y = ga.run()

formatted_x = [round(float(x), 3) for x in best_x]
formatted_y = round(float(-best_y[0]), 2)
formatted_y_hist = [-float(y[0]) for y in ga.all_history_Y]

print('第三问：遗传算法最佳参数-值')
...
plt.ylabel('Best Y')
plt.title('q3:GA Optimization History')
plt.show()
...
# data_to_fill = {
#     '无人机运动方向': [fmt(best_x[1], 3), fmt(best_x[5], 3), fmt(best_x[9], 3)],
#     '无人机运动速度 (m/s)': [fmt(best_x[0], 3), fmt(best_x[4], 3), fmt(best_x[8], 3)],
